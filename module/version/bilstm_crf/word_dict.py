...
class WordDict(Dictionary):
    ...
    def fit(self, data, identifiers):
        ...
        self.word_token[self.PAD_TAG] = len(self.word_token)

        for identifier, _ in identifier_counter.items():
            self.identifier_token[identifier] = len(self.identifier_token)
        self.identifier_token[self.PAD_TAG] = len(self.identifier_token)
...
def test_module_func():
    ...
    assert seq == [3, 4, 5]
    assert word_dict.identifier_to_token(word_dict.START_TAG) == 0
    assert word_dict.identifier_to_token(word_dict.END_TAG) == 1
    assert word_dict.decoding_identifier([3, 4, 5]) == ['B_game', 'I_game', 'E_game']
