...
@desc:
# 训练：使用训练数据集进行训练
    example: python ner.py train

# 评估：将使用 dev 数据集进行评估。输出损失
    example: python ner.py eval

# 评分: 使用 dev 数据集进行预测评分
    example: python ner.py score

# 测试：将使用测试数据集，将测试数据集识别结果写入文件
    example：python ner.py test --file "predict_result.json"
            --file          指定写入的文件名
# 预测：
    example: python ner.py prediction --sentence "《加勒比海盗3：世界尽头》的去年同期成绩死死甩在身后，后者则即将赶超《变形金刚》，"
            --sentence      指定预测的句子
"""
import re
import sys
...
try:
    ...
    from module.version.bilstm_crf.word_dict import WordDict
    from module.core.color import Color
    from module.version.bilstm_crf.bilstm_crf import NER_BiLSTM_CRF
...
def init_bilstm_crf(vocab_size, target_size, start_index, end_index, conf):
    """
    初始化 BiLSTM_CRF 模型
    :param vocab_size: (int, mandatory) 输入词汇量大小
    :param target_size: (int, mandatory) 目标词汇输出大小
    ...
    """
    model = NER_BiLSTM_CRF(embed_size=conf.MODEL.BILSTM_CRF.embed_size,
                           vocab_size=vocab_size,
                           target_size=target_size,
                           lstm_units=conf.MODEL.BILSTM_CRF.lstm_units,
                           start_index=start_index,
                           end_index=end_index,
                           l1=conf.MODEL.BILSTM_CRF.l1,
                           l2=conf.MODEL.BILSTM_CRF.l2,
                           merge_mode=conf.MODEL.BILSTM_CRF.merge_mode,
                           learning_rate=conf.MODEL.BILSTM_CRF.learning_rate,
                           polynomial_decay_steps=conf.MODEL.BILSTM_CRF.polynomial_decay_steps,
                           polynomial_end_learning_rate=conf.MODEL.BILSTM_CRF.polynomial_end_learning_rate,
                           polynomial_power=conf.MODEL.BILSTM_CRF.polynomial_power,
                           momentum=conf.MODEL.BILSTM_CRF.momentum,
                           decay=conf.MODEL.BILSTM_CRF.decay,
                           use_polynomial_decay=conf.MODEL.BILSTM_CRF.use_polynomial_decay,
                           checkpoint_dir=conf.MODEL.BILSTM_CRF.checkpoint_dir,
                           max_to_keep=conf.MODEL.BILSTM_CRF.max_to_keep)
    return model


def init_or_restore_bilstm_crf(file, checkpoint_dir, vocab_size, target_size, start_index, end_index, conf):
    """
    初始化或还原 BiLSTM_CRF 模型。当发现配置文件和检查点目录时候，则尝试从配置文件和检查点恢复模型
    :param file: (str, mandatory) 配置文件
    :param checkpoint_dir: (str, mandatory) 检查点目录
    :param vocab_size: (int, mandatory) 输入词汇量大小
    :param target_size: (int, mandatory) 目标词汇输出大小
    :param start_index: (int, mandatory) 序列开始<STARt>索引
    :param end_index: (int, mandatory) 序列结束<END>索引
    ...
    :return: (NER_BiLSTM_CRF) 模型对象
    """
    if os.path.isfile(file) and tf.train.latest_checkpoint(checkpoint_dir):
        model = NER_BiLSTM_CRF.restore(file, checkpoint_dir)
        output = "Restore model from:{} and checkpoint:{} ".format(file, checkpoint_dir)
    else:
        model = init_bilstm_crf(vocab_size, target_size, start_index, end_index, conf)
        output = "Not found config file:{} and checkpoint:{}, Initializing from scratch".format(file, checkpoint_dir)
...
class NameEntityRecognition(object):
    ...
    def __init_dataset__(self):
        ...
        if self.mode == M_TRAIN or self.mode == M_SCORE or self.mode == M_EVAL:
            ...
            for text, identifier in zip(text_data, identifier_data):
                x, _ = self.word_dict.encoding_sentence(text, alignment=False)
                y, _ = self.word_dict.encoding_identifier(identifier, alignment=False)
    ...
    def __init_model__(self):
        """ 初始化模型 """
        start_index = self.word_dict.identifier_to_token(self.word_dict.START_TAG)
        end_index = self.word_dict.identifier_to_token(self.word_dict.END_TAG)
        self.model = init_or_restore_bilstm_crf(file=self.conf.MODEL.BILSTM_CRF.PATH_CONFIG,
                                                checkpoint_dir=self.conf.MODEL.BILSTM_CRF.checkpoint_dir,
                                                vocab_size=len(self.word_dict.word_token),
                                                target_size=len(self.word_dict.identifier_token),
                                                start_index=start_index,
                                                end_index=end_index,
                                                conf=self.conf)
    ...
    def train(self):
        ...
        self.model.fit(x, y,
                       ...
                       validation_split=self.conf.MODEL.BILSTM_CRF.VALIDATION_SPLIT)
        self.model.dump_config(self.conf.MODEL.BILSTM_CRF.PATH_CONFIG)
    ...
    def score(self):
        ...
        for i in range(len(texts)):
            x, _ = self.word_dict.encoding_sentence(texts[i], alignment=False)
            prediction = self.model.predict(x)
            pred_identifier = self.word_dict.decoding_identifier(prediction)
    ...
    def predict(self, sentence):
        ...
        x, _ = self.word_dict.encoding_sentence(list(sentence))
        prediction = self.model.predict(x)
        pred_identifier = self.word_dict.decoding_identifier(prediction)
...
def test_module_func(operation):
    if operation == 'plot_decay':
        lr = 0.01
        _decay = tf.keras.optimizers.schedules.PolynomialDecay(lr, 10000, power=0.1)
        plt.figure(figsize=(8, 16))
        plt.plot(_decay(tf.range(10000, dtype=tf.float32)))
        plt.show()

    elif operation == load_train_or_dev_dataset.__name__:
